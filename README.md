...
| --- | --- | --- |
| Mac with an Apple M-series chip | `Burrow-0.1.0-macOS-AppleSilicon.dmg` | Open it, drag **Burrow** into **Applications**, then open Burrow there. |
| Mac with an Intel processor | `Burrow-0.1.0-macOS-Intel.dmg` | Open it, drag **Burrow** into **Applications**, then open Burrow there. |
| Windows PC with an Intel or AMD 64-bit processor | `Burrow-0.1.0-Windows-x64-Setup.exe` | Open it, choose **Install**, then launch **Burrow** from Start. No administrator install is required. |
...
**[Read the illustrated-by-steps installation, warning and recovery guide →](docs/INSTALL.md)**
...
The Rust executable draws through egui/OpenGL. There is no embedded Chromium engine and no JavaScript bridge. CPU/memory sampling happens on a background worker every two seconds; disk information refreshes every ten seconds. At idle, the GUI redraws for input and fresh samples rather than running a continuous animation loop.
...
For a **reproducible release build**, download that release's `Burrow-0.1.0-source.zip`, which includes its exact `Cargo.lock`, and use `cargo build --release --locked`. The initial workflow resolves dependencies once and shares the same lock across all build jobs. The lock is also attached separately to the release. Future maintainers should commit a reviewed lockfile before updating dependencies.
...
src/monitor.rs      Bounded-channel CPU/RAM/disk monitor
