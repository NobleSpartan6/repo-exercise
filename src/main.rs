...
fn main() -> std::process::ExitCode {
    ...
    match eframe::run_native("Burrow", options, Box::new(move |cc| {
        #[cfg(any(target_os = "windows", target_os = "macos"))]
...
fn native_gpu() -> eframe::egui_wgpu::WgpuConfiguration {
    ...
    let setup = WgpuSetupCreateNew {
        instance_descriptor: wgpu::InstanceDescriptor {
            backends: if cfg!(target_os = "macos") { wgpu::Backends::METAL } else { wgpu::Backends::DX12 },
            ..Default::default()
        },
        power_preference: wgpu::PowerPreference::LowPower,
        ..Default::default()
    };
    WgpuConfiguration { wgpu_setup: setup.into(), ..Default::default() }
}
    ...
    for y in 0..size {
        for x in 0..size {
            ...
            rgba.extend_from_slice(if !outer { &[0, 0, 0, 0] }
                else if tunnel && !inner { &[151, 228, 192, 255] }
                else { &[17, 21, 25, 255] });
        }
    }
