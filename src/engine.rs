...
impl Control {
    ...
    pub fn cancelled(&self) -> bool { self.cancel.load(Ordering::Relaxed) }
...
impl Stamp {
    fn read(meta: &Metadata) -> Result<Self, String> {
        ...
        #[cfg(unix)]
        {
            ...
            if meta.nlink() != 1 { return Err("Hard-linked files are excluded".into()); }
        }
        Ok(Self {
            bytes: meta.len(),
            ...
            #[cfg(unix)]
            device: { use std::os::unix::fs::MetadataExt; meta.dev() },
            #[cfg(unix)]
...
fn old_enough(stamp: &Stamp, age: Duration, now: SystemTime) -> bool {
    now.duration_since(stamp.modified).map(|d| d >= age).unwrap_or(false)
}
...
pub fn preview(roots: &[CacheRoot], days: u32, control: &Control) -> Result<Preview, String> {
    ...
    for root in roots {
        if !root.path.try_exists().unwrap_or(true) { continue; }
        match checked_path(&root.path) {
            Ok(path) if path.is_dir() => unique.push(CacheRoot { label: root.label.clone(), path }),
    ...
    for root in unique {
        if !accepted.iter().any(|r| root.path.starts_with(&r.path)) { accepted.push(root); }
    }
    let pool = rayon::ThreadPoolBuilder::new().num_threads(4)
        .thread_name(|n| format!("burrow-scan-{n}"))
        .build().map_err(|e| e.to_string())?;
    let parts: Vec<Preview> = pool.install(|| accepted.par_iter().map(|root| {
        let mut result = Preview::default();
        let mut walker = WalkDir::new(&root.path).follow_links(false)
            .same_file_system(true).max_depth(MAX_DEPTH).into_iter();
        while let Some(entry) = walker.next() {
            if !control.advance(start) || count.load(Ordering::Relaxed) >= MAX_CANDIDATES {
                result.partial = true;
                break;
            }
            let entry = match entry { Ok(e) => e, Err(_) => { result.skipped += 1; continue; } };
            let meta = match fs::symlink_metadata(entry.path()) {
                Ok(m) => m, Err(_) => { result.skipped += 1; continue; }
            };
            if is_link_or_placeholder(&meta) {
            ...
            if meta.is_dir() {
                if entry.depth() == MAX_DEPTH { result.partial = true; }
                continue;
            }
            let stamp = match Stamp::read(&meta) {
                Ok(s) => s, Err(_) => { result.skipped += 1; continue; }
            };
            if stamp.bytes == 0 || !old_enough(&stamp, age, now) { continue; }
            if count.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |n| {
                (n < MAX_CANDIDATES).then_some(n + 1)
            }).is_err() { result.partial = true; break; }
            result.files.push(Candidate {
                path: entry.path().to_path_buf(), root: root.path.clone(),
                category: root.label.clone(), bytes: stamp.bytes, stamp, min_age: age,
            });
        }
        result
    }).collect());
    let mut result = Preview { warnings, ..Preview::default() };
    for part in parts {
        result.files.extend(part.files);
        result.skipped += part.skipped;
        result.partial |= part.partial;
    }
    result.files.sort_unstable_by(|a, b| b.bytes.cmp(&a.bytes).then(a.path.cmp(&b.path)));
...
fn validate(candidate: &Candidate, roots: &[PathBuf]) -> Result<(), String> {
    if !roots.contains(&candidate.root) { return Err("Cache is no longer allowlisted".into()); }
...
fn clean_with<F>(files: &[Candidate], roots: &[CacheRoot], control: &Control, mut recycle: F) -> Cleanup
where F: FnMut(&Path) -> Result<(), String> {
    let allowed: Vec<PathBuf> = roots.iter().filter_map(|r| checked_path(&r.path).ok()).collect();
    let mut report = Cleanup::default();
    let mut seen = HashSet::new();
    for file in files {
        if control.cancelled() { report.cancelled = true; break; }
        if !seen.insert(&file.path) { continue; }
        ...
        match validate(file, &allowed).and_then(|()| recycle(&file.path)) {
            Ok(()) => {
                ...
                report.log.push(format!("MOVED TO TRASH\t{}\t{} bytes", file.path.display(), file.bytes));
            }
            Err(error) => {
                report.refused += 1;
                report.log.push(format!("NOT CONFIRMED MOVED\t{}\t{error}", file.path.display()));
            }
        }
    }
    report
...
pub fn clean_selected(files: &[Candidate], control: &Control) -> Cleanup {
    clean_with(files, &platform::cache_roots(), control, |path| {
        trash::delete(path).map_err(|e| format!("Trash operation failed: {e}. Inspect Trash before retrying."))
    })
}

#[derive(Debug)]
pub struct LargeFile { pub path: PathBuf, pub bytes: u64 }
...
pub fn analyze(folder: &Path, control: &Control) -> Result<Analysis, String> {
    ...
    let start = Instant::now();
    let mut result = Analysis { root: root.clone(), ..Analysis::default() };
    let mut top = BinaryHeap::new();
    let mut walker = WalkDir::new(&root).follow_links(false).same_file_system(true)
        .max_depth(MAX_DEPTH).into_iter();
    while let Some(entry) = walker.next() {
        if !control.advance(start) { result.partial = true; break; }
        let entry = match entry { Ok(e) => e, Err(_) => { result.skipped += 1; continue; } };
        let meta = match fs::symlink_metadata(entry.path()) {
            Ok(m) => m, Err(_) => { result.skipped += 1; continue; }
        };
        ...
        if !meta.is_file() { result.skipped += 1; continue; }
        result.files += 1;
        result.total_bytes = result.total_bytes.saturating_add(meta.len());
        top.push(Reverse((meta.len(), entry.path().to_path_buf())));
        if top.len() > TOP_FILES { top.pop(); }
    }
    result.top = top.into_iter().map(|Reverse((bytes, path))| LargeFile { path, bytes }).collect();
    result.top.sort_unstable_by(|a, b| b.bytes.cmp(&a.bytes).then(a.path.cmp(&b.path)));
    result.elapsed = start.elapsed();
    result.visited = control.visited.load(Ordering::Relaxed);
    result.cancelled = control.cancelled();
    result.partial |= result.cancelled;
...
mod tests {
    ...
    fn fixture() -> (TempDir, CacheRoot) {
        ...
        let root = CacheRoot { label: "Test-only cache".into(), path: dir.path().canonicalize().unwrap() };
        (dir, root)
    }
    fn old_file(root: &CacheRoot, name: &str, bytes: u64) -> PathBuf {
        ...
        set_file_mtime(&path, FileTime::from_system_time(SystemTime::now() - Duration::from_secs(30 * DAY))).unwrap();
        path
    }
    fn plan(root: &CacheRoot) -> Vec<Candidate> {
        preview(std::slice::from_ref(root), 7, &Control::default()).unwrap().files
    }
    #[test]
    fn excludes_recent_empty_and_future_files() {
        ...
        set_file_mtime(future, FileTime::from_system_time(SystemTime::now() + Duration::from_secs(DAY))).unwrap();
        let files = plan(&root);
    ...
    fn exact_age_boundary() {
        ...
        assert!(old_enough(&files[0].stamp, Duration::from_secs(7 * DAY), at_boundary));
        assert!(!old_enough(&files[0].stamp, Duration::from_secs(7 * DAY), at_boundary - Duration::from_secs(1)));
    }
    #[test]
    fn changing_file_invalidates_preview() {
        ...
        let report = clean_with(&files, &[root], &Control::default(), |_| panic!("must not recycle"));
    ...
    fn file_outside_allowlist_cannot_be_recycled() {
        ...
        old_file(&root, "old", 1);
        let files = plan(&root);
        let report = clean_with(&files, &[], &Control::default(), |_| panic!("must not recycle"));
        assert_eq!(report.refused, 1);
    }
    #[test]
    ...
    fn native_trash_failure_never_calls_permanent_delete() {
        ...
        let files = plan(&root);
        let report = clean_with(&files, &[root], &Control::default(), |_| Err("Trash unavailable".into()));
        assert_eq!(report.refused, 1);
    ...
    fn duplicate_selection_is_processed_once() {
        ...
        let mut calls = 0;
        let report = clean_with(&files, &[root], &Control::default(), |_| { calls += 1; Ok(()) });
        assert_eq!(calls, 1);
    ...
    fn cancellation_is_not_success() {
        ...
        control.stop();
        let report = clean_with(&files, std::slice::from_ref(&root), &control, |_| panic!("must not recycle"));
    ...
    fn overlapping_roots_do_not_duplicate_candidates() {
        ...
        let child = CacheRoot { label: "Child".into(), path: root.path.join("nested") };
        assert_eq!(preview(&[root.clone(), child, root], 7, &Control::default()).unwrap().files.len(), 1);
    }
    #[test]
    fn analysis_is_sorted_bounded_and_read_only() {
        let (_dir, root) = fixture();
        for i in 1..=250 { old_file(&root, &format!("{i}.cache"), i); }
    ...
    fn symlink_root_and_ancestor_are_rejected() {
        ...
        let link = CacheRoot { label: "Link".into(), path: root.path.join("linked") };
    ...
    fn windows_junction_is_not_followed() {
        ...
        let output = Command::new("cmd").args(["/C", "mklink", "/J"])
            .arg(&junction).arg(&outside.path).output().unwrap();
