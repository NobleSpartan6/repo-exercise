...
pub fn configure(ctx: &egui::Context) {
    ...
    for (name, paths, family) in [
        ...
        ("burrow-bold", bold, FontFamily::Name("burrow-heading".into())),
    ] {
        ...
        for path in paths {
            ...
            {
                fonts.font_data.insert(name.into(), egui::FontData::from_owned(data).into());
    ...
    ctx.style_mut(|style| {
        ...
        style.text_styles.insert(egui::TextStyle::Heading, heading(28.0));
...
pub fn card() -> egui::Frame {
    egui::Frame::new().fill(PANEL).stroke(Stroke::new(1.0, LINE))
        .corner_radius(16).inner_margin(20)
}
...
pub fn button(ui: &mut egui::Ui, text: &str, enabled: bool, primary: bool) -> egui::Response {
    let widget = egui::Button::new(RichText::new(text).color(if primary { ACCENT_INK } else { TEXT }))
...
pub fn record(response: &egui::Response, _name: &str) {
    #[cfg(test)]
    response.ctx.data_mut(|data| data.insert_temp(egui::Id::new(_name), response.rect));
    #[cfg(not(test))]
    let _ = response;
}
...
pub fn orbit(ui: &mut egui::Ui, size: f32) {
    ...
    let p = ui.painter();
    for (radius, color) in [(0.46, LINE), (0.35, Color32::from_rgb(66, 113, 94)), (0.23, ACCENT)] {
        p.circle_stroke(center, size * radius, Stroke::new(if radius < 0.3 { 2.0 } else { 1.0 }, color));
    }
    let angle: f32 = -0.72;
    p.circle_filled(center + egui::vec2(angle.cos(), angle.sin()) * size * 0.35, 4.0, ACCENT);
...
pub fn bar(ui: &mut egui::Ui, fraction: f32, color: Color32) {
    ...
    ui.add(egui::ProgressBar::new(fraction.clamp(0.0, 1.0)).fill(color)
        .desired_width(ui.available_width()).desired_height(5.0));
}
