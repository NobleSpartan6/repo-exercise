...
impl Totals {
    fn decode(bytes: &[u8]) -> Result<Self, String> {
        ...
        let number = |key: &str| value[key].as_u64().ok_or_else(|| format!("Invalid cleanup total: {key}"));
        Ok(Self { runs: number("runs")?, files: number("files")?, bytes: number("bytes")? })
    }
    fn add(self, result: &Cleanup) -> Result<Self, String> {
        if result.moved == 0 {
            return Ok(self);
        }
        let add = |a: u64, b: u64| a.checked_add(b).ok_or_else(|| "Cleanup total exceeded its limit.".to_owned());
        Ok(Self {
...
fn path() -> Result<PathBuf, String> {
    dirs::data_local_dir().map(|p| p.join("Burrow/cleanup-totals.json")).ok_or_else(|| "No local folder for cleanup totals.".into())
}
fn load_at(path: &Path) -> Result<Totals, String> {
    match fs::symlink_metadata(path) {
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Ok(Totals::default()),
        Err(e) => return Err(e.to_string()),
        Ok(meta) if !meta.is_file() || engine::is_link_or_placeholder(&meta) => return Err("Cleanup totals must be a regular local file.".into()),
        Ok(_) => {}
    }
    engine::checked_path(path)?;
    let mut bytes = Vec::new();
    File::open(path).map_err(|e| e.to_string())?.take(MAX_BYTES + 1).read_to_end(&mut bytes).map_err(|e| e.to_string())?;
    Totals::decode(&bytes)
}
...
fn record_at(path: &Path, result: &Cleanup) -> Result<Totals, String> {
    ...
    let lock = OpenOptions::new().create(true).truncate(false).read(true).write(true).open(lock_path).map_err(|e| e.to_string())?;
    ...
    lock.try_lock().map_err(|e| format!("Cleanup totals are busy; this run was not recorded: {e}"))?;
    let total = load_at(path)?.add(result)?;
    let bytes = serde_json::to_vec(&json!({"version": 1, "runs": total.runs, "files": total.files, "bytes": total.bytes})).map_err(|e| e.to_string())?;
...
mod tests {
    use super::*;
    fn result() -> Cleanup {
        Cleanup { moved: 2, moved_bytes: 30, refused: 9, cancelled: true, log: vec!["PRIVATE/PATH".into()] }
    }
    #[test]
    fn only_confirmed_moves_accumulate_and_paths_are_not_stored() {
        ...
        assert_eq!(record_at(&path, &result()).unwrap(), Totals { runs: 1, files: 2, bytes: 30 });
        assert_eq!(record_at(&path, &result()).unwrap(), Totals { runs: 2, files: 4, bytes: 60 });
    ...
    fn empty_or_failed_run_creates_no_totals() {
        let d = tempfile::tempdir_in(std::env::temp_dir().canonicalize().unwrap()).unwrap();
        let path = d.path().join("totals.json");
        let empty = Cleanup { moved: 0, moved_bytes: 0, refused: 1, cancelled: false, log: vec![] };
    ...
    fn corrupt_newer_or_oversized_totals_are_preserved() {
        let d = tempfile::tempdir_in(std::env::temp_dir().canonicalize().unwrap()).unwrap();
        let path = d.path().join("totals.json");
        for text in ["invalid".to_owned(), r#"{"version":2}"#.into(), " ".repeat(4097)] {
    ...
    fn counts_do_not_wrap() {
        let totals = Totals { runs: u64::MAX, files: 0, bytes: 0 };
